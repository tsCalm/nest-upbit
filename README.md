...
- Candle extends BaseEntity로 변경 후 데이터베이스에 해당 캔들 정보가 존재하는지 검사 후 존재한다면 덮어쓰고 없다면 새로 저장하는 방식으로 변경해야 함 업비트에서 보내주는 response에 primary키로 사용할만한 데이터가 존재하지 않음 -> 대량의 데이터를 빠르게 처리해야하기 때문에 인덱싱에 신경써야한다. 기준 데이터부터 정하고 작업하기
